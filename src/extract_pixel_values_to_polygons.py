...
class PolygonGeometry:
    ...
    def __init__(self, this_line_geometry, array, array_tile, tile_shape):

        self.vertexes = tuple(zip(this_line_geometry.xy[0], this_line_geometry.xy[1]))

        self.bounding_box = (
            ((min(this_line_geometry.xy[0]), min(this_line_geometry.xy[1])), (min(this_line_geometry.xy[0]), max(this_line_geometry.xy[1]))),
            ((max(this_line_geometry.xy[0]), min(this_line_geometry.xy[1])), (max(this_line_geometry.xy[0]), max(this_line_geometry.xy[1])))
        )

        self.tile_bounding_box = [[[0, 0], [0, 0]], [[0, 0], [0, 0]]]

        self.pixel_bounding_box = [[[0, 0], [0, 0]], [[0, 0], [0, 0]]]

        self.get_tile_and_pixel_bounding_box(array_tile=array_tile, tile_shape=tile_shape)

        self.vertex_tiles = ()

        self.vertex_pixels = ()

        self.get_vertex_tiles_and_pixels()
        ...
        self.all_unique_pixels_along_polygon_boundary = []

        self.get_all_unique_pixels_along_polygon_boundary()

        self.array_values_along_line_segments = []

        self.extract_pixel_values_to_line_segments(array=array, array_tile=array_tile)

        self.pixel_polygon_concave_hull = []

        self.get_pixels_in_polygon_concave_hull()

        self.array_values_for_concave_hull = []

        self.extract_pixel_values_to_concave_hull(array=array)

    def get_tile_and_pixel_bounding_box(self, array_tile, tile_shape):

        for m in range(0, 2):

            for n in range(0, 2):

                self.tile_bounding_box[m][n][0], self.pixel_bounding_box[m][n][0] = get_tile_pixel_from_long(
                    self.bounding_box[m][n][0]
                )

                self.tile_bounding_box[m][n][1], self.pixel_bounding_box[m][n][1] = get_tile_pixel_from_lat(
                    self.bounding_box[m][n][1]
                )
                
                # The following code makes sure that we only look at pixels within the particular tile we have chosen

                if tuple(self.tile_bounding_box[m][n]) != array_tile:

                    if self.tile_bounding_box[m][n][0] > array_tile[0]:
                        # want to set to maximum x-axis coordinate for grid
                        self.pixel_bounding_box[m][n][0] = tile_shape[1] - 1

                    if self.tile_bounding_box[m][n][0] < array_tile[0]:
                        self.pixel_bounding_box[m][n][0] = 0

                    if self.tile_bounding_box[m][n][1] > array_tile[1]:
                        self.pixel_bounding_box[m][n][1] = tile_shape[0] - 1

                    if self.tile_bounding_box[m][n][1] < array_tile[1]:
                        self.pixel_bounding_box[m][n][1] = 0

                    # Now change tile to the correct one we want to look in
                    # Want to get a deep copy of array tile

                    self.tile_bounding_box[m][n] = list(array_tile)

        # Make sure following code works correctly

        # The following makes sure that we have a rectangular shape where opposite sides are parallel and the same length
        # (Though there is a possibility that we get a triangle, line or point depending on how many of the pixel vertexes
        #   that make the bounding box are equal).

        # Take the smallest of the y values at the bottom and set both y values to be equal to this value

        self.pixel_bounding_box[0][0][1] = min([self.pixel_bounding_box[0][0][1], self.pixel_bounding_box[1][0][1]])

        self.pixel_bounding_box[1][0][1] = self.pixel_bounding_box[0][0][1]

        # Do same but for the max with the top y values

        self.pixel_bounding_box[0][1][1] = max([self.pixel_bounding_box[0][1][1], self.pixel_bounding_box[1][1][1]])

        self.pixel_bounding_box[1][1][1] = self.pixel_bounding_box[0][1][1]

        # Now for the x values ...

        self.pixel_bounding_box[0][0][0] = min([self.pixel_bounding_box[0][0][0], self.pixel_bounding_box[0][1][0]])

        self.pixel_bounding_box[0][1][0] = self.pixel_bounding_box[0][0][0]

        self.pixel_bounding_box[1][0][0] = max([self.pixel_bounding_box[1][0][0], self.pixel_bounding_box[1][1][0]])

        self.pixel_bounding_box[1][1][0] = self.pixel_bounding_box[1][0][0]

    def get_vertex_tiles_and_pixels(self):
    ...
    def get_all_unique_pixels_along_polygon_boundary(self):

        for i in range(len(self.pixel_line_segments)):

            for point_pixel in self.pixel_line_segments[i]:

                self.all_unique_pixels_along_polygon_boundary.append(point_pixel)

        self.all_unique_pixels_along_polygon_boundary = tuple(set(self.all_unique_pixels_along_polygon_boundary))
    ...
    def get_pixels_in_polygon_concave_hull(self):

        unique_pixel_bounding_box_vertexes = tuple(
            {tuple(self.pixel_bounding_box[0][0]), tuple(self.pixel_bounding_box[0][1]),
             tuple(self.pixel_bounding_box[1][0]), tuple(self.pixel_bounding_box[1][1])}
        )

        # if bounding box is really a point
        if len(unique_pixel_bounding_box_vertexes) == 1:
            self.pixel_polygon_concave_hull = tuple(unique_pixel_bounding_box_vertexes)

        # if bounding box is really a line segment
        elif len(unique_pixel_bounding_box_vertexes) == 2:

            self.pixel_polygon_concave_hull = tuple(
                set(
                    find_pixels_along_line_segment(
                        pixel_one=unique_pixel_bounding_box_vertexes[0],
                        pixel_two=unique_pixel_bounding_box_vertexes[1]
                    )
                )
            )

        # if bounding box is really a triangle
        elif len(unique_pixel_bounding_box_vertexes) == 3:

            for m, n in [(0,1), (1, 2), (2, 0)]:

                self.pixel_polygon_concave_hull += list(
                    find_pixels_along_line_segment(
                        pixel_one=unique_pixel_bounding_box_vertexes[m],
                        pixel_two=unique_pixel_bounding_box_vertexes[n]
                    )
                )

            self.pixel_polygon_concave_hull = tuple(set(self.pixel_polygon_concave_hull))

        # if bounding box is rectangle
        elif len(unique_pixel_bounding_box_vertexes) == 4:

            # checking to make sure looking at rectangle
            assert self.pixel_bounding_box[0][1][1] == self.pixel_bounding_box[1][1][1]
            assert self.pixel_bounding_box[0][0][1] == self.pixel_bounding_box[1][0][1]
            assert self.pixel_bounding_box[0][0][1] != self.pixel_bounding_box[0][1][1]
            assert self.pixel_bounding_box[1][0][1] != self.pixel_bounding_box[1][1][1]

            # Scan the bounding box from bottom to top
            for n in range(self.pixel_bounding_box[0][0][1], self.pixel_bounding_box[0][1][1] + 1):

                this_scan_segment = find_pixels_along_line_segment(
                    pixel_one=(self.pixel_bounding_box[0][0][0], n),
                    pixel_two=(self.pixel_bounding_box[1][0][0], n)
                )

                scan_segment_pixel_points_on_polygon_boundary = []

                for pixel_point in this_scan_segment:

                    if pixel_point in self.all_unique_pixels_along_polygon_boundary:

                        scan_segment_pixel_points_on_polygon_boundary.append(True)

                    elif pixel_point not in self.all_unique_pixels_along_polygon_boundary:
                ...
                for i in range(len(scan_segment_pixel_points_on_polygon_boundary)):

                    boundary_flag = scan_segment_pixel_points_on_polygon_boundary[i]

                    # if pixel_point is on boundary
                    if boundary_flag:
                        ...
                        if counter % 2 != 0:
                            ...
                            left_boundary_point = tuple(this_scan_segment[i])

                        elif counter % 2 == 0:

                            right_boundary_point = tuple(this_scan_segment[i])

                            # finding part of scan segment in polygon

                            self.pixel_polygon_concave_hull += list(
                                find_pixels_along_line_segment(
                                    pixel_one=left_boundary_point,
                                    pixel_two=right_boundary_point
                                )
                            )
                # Counter equals 1 right_boundary_point is empty tuple
                if counter == 1 and not right_boundary_point:
                    self.pixel_polygon_concave_hull = [left_boundary_point]

        self.pixel_polygon_concave_hull = tuple(set(self.pixel_polygon_concave_hull))

    def extract_pixel_values_to_concave_hull(self, array):

        for point_pixel in self.pixel_polygon_concave_hull:
            # Check that pixel point is not empty tuple
            if point_pixel:

                self.array_values_for_concave_hull.append(array[point_pixel[1], point_pixel[0]])

            elif not point_pixel:
                self.array_values_for_concave_hull.append(np.nan)

        self.array_values_for_concave_hull = tuple(self.array_values_for_concave_hull)



# Figure out why some polygons don't have a concave hull (self.pixel_polygon_concave_hull is an empty tuple)
#   They must not be polygons at all but are just lines or points even though bounding box is rectangle
#   The `counter` variable probably equals zero for these polygons
...
def extract_pixel_values_to_polygon(line_gp_df, array, array_tile=(11, 7)):

    geometry = gpd.GeoSeries(line_gp_df['geometry'])

    # Shapely geometry types: https://shapely.readthedocs.io/en/stable/geometry.html

    if sum(geometry.geometry.type == 'Polygon') != len(geometry.geometry.type == 'Polygon'):
        # Don't put in try-except block if you want the error to fail loudly
        raise ValueError('Not all feature geometries are Polygon geometries.')
    ...
    extractions = {
        ...
        "pixel_polygon_concave_hull": [],
        "array_values_for_concave_hull": []
    }

    for this_polygon in geometry_as_dict.values():

        this_polygon_geometry = PolygonGeometry(this_line_geometry=this_polygon.boundary, array=array, array_tile=array_tile, tile_shape=array.shape)

        extractions["vertex_tiles"].append(this_polygon_geometry.vertex_tiles)

        extractions["vertex_pixels"].append(this_polygon_geometry.vertex_pixels)

        extractions["line_segments_contained_in_tile"].append(this_polygon_geometry.line_segments_contained_in_tile)

        extractions["pixel_line_segments"].append(this_polygon_geometry.pixel_line_segments)

        extractions["array_values_along_line_segments"].append(this_polygon_geometry.array_values_along_line_segments)

        extractions["pixel_polygon_concave_hull"].append(this_polygon_geometry.pixel_polygon_concave_hull)

        extractions["array_values_for_concave_hull"].append(this_polygon_geometry.array_values_for_concave_hull)
    ...
    assert (len(extractions["index"]) == len(extractions["vertex_tiles"]) == len(extractions["vertex_pixels"]) ==
           ...
           len(extractions["array_values_along_line_segments"]) == len(extractions["pixel_polygon_concave_hull"]) ==
            len(extractions["array_values_for_concave_hull"]) == line_gp_df.shape[0])
...
if __name__ == '__main__':
    ...
    # Todo: Make sure polygon and line scripts still run with the edits you made
    #   Stardardize the way that you subset to a particular tile across files
    ...
    if True:
        ...
        substation_filepath = Path('/Users/brevi/Documents/All_Files/Harvard_GSAS_RSI/Senior_Thesis/Data/Infrastructure/Electricity_Transmission_and_Distribution/g37_electric_base_de_subestaciones_2014/g37_electric_base_de_subestaciones_2014.shp')
        # open in geopandas
        ss_gdf = gpd.read_file(substation_filepath)

        test_ss_gdf_with_extraction_df = extract_pixel_values_to_polygon(
            line_gp_df=ss_gdf,
            array=test_viirs_image.filtered_array,
            array_tile=(11, 7)
        )

        # print(test_ss_gdf_with_extraction_df)
    ...
    if False:
        ...
        save_as_pickle(
            a_file=test_ss_gdf_with_extraction_df,
            path=Path(OUTPUT_PATH, "test_ss_gdf_with_extraction_df.pkl")
        )
...
#  Start thinking about how to handle points and lines
    ...
    # Not all polygons will lie entirely in one pixel

    # for element in ss_gdf_geometry.to_dict().values():
    #
    #     print(
    #         PolygonGeometry(
    #             this_line_geometry=element.boundary,
    #             array=test_viirs_image.filtered_array, array_tile=(11, 7),
    #             tile_shape=test_viirs_image.filtered_array.shape
    #         ).pixel_bounding_box
    #     )
...
# Todo: Looking into using Rasterio to convert Numpy array to raster
#       https://gis.stackexchange.com/questions/279953/numpy-array-to-gtiff-using-rasterio-without-source-raster
#   Also, see how to extract Rasterio raster to geopandas vector
#       Look at Raster stats and Rasterio:
#        https: // pythonhosted.org / rasterstats / manual.html
#        https://geopandas.org/en/stable/gallery/geopandas_rasterio_sample.html
#   See how this method compares to the results get using your method